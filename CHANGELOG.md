...
- `.mcp.json` declaring the remote `design-extractor` MCP server at `https://www.design-extractor.com/api/mcp` over streamable HTTP. The agent signs in with the user's Design Extractor account through OAuth on first use; no key or environment variable.
